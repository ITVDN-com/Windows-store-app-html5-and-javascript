...
(function () {
    ...
    app.onloaded = function () {
        WinJS.UI.Animation.enterPage(root, { top: '0px', left: '1000px' });
    }

    function loaded() {
        // winControl - объект представляющий WinJS элемент управления
        rating = RatingControl.winControl;
        coursesListView = CoursesListView.winControl;

        rating.addEventListener('change', change);
        SubmitButton.addEventListener('click', submit);

        
        coursesListView.addEventListener("iteminvoked", iteminvoked);
    }

    function iteminvoked(eventInfo) {
        // запуск анимации fadeOut на элементе ratingOutput и запуск функции сразу после завершения анимации
        WinJS.UI.Animation.fadeOut(ratingOutput).then(function () {
            ...
            // запуск анимации fadeIn на элементе ratingOutput
            WinJS.UI.Animation.fadeIn(ratingOutput);
        });
    }

    function change(e) {
        var current = DataExample.itemList.getAt(coursesListView.currentItem.index);
